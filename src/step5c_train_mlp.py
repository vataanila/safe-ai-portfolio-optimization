...
step5c_train_mlp.py
=============
PURPOSE : MLPRegressor-based expected return estimation (mu_hat).

          Trains a neural network in an expanding-window OOS framework over
          the rebalancing dates defined by baseline_weights.csv.
...
OUTPUTS :
  data/results/step5/ml_mu_mlp.csv
  data/results/step5/diagnostics/mlp/
    mlp_ic_detail.csv
    mlp_ic_summary.csv
...
DIAG_DIR    = os.path.join(STEP5_DIR, "diagnostics", "mlp")
...
def train_mlp(X_tr, y_tr, X_vl, y_vl):
    """
    Train MLP on train+val combined (sklearn handles its own early-stopping
    holdout internally).  Fixed architecture, no hyperparameter search.
...
def main():
    ...
    print("=" * 70)
    print("  STEP 5C â€” MLP EXPECTED RETURN ESTIMATION (MU_HAT)")
    print("=" * 70)
    ...
    out_path = os.path.join(STEP5_DIR, "ml_mu_mlp.csv")
  ...
  Next step: run step5a_ridge.py or step5b_xgboost.py.
""")
