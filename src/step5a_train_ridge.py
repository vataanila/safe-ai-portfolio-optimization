...
===============
PURPOSE : Ridge Regression-based expected return estimation (mu_hat).

          Trains Ridge in an expanding-window OOS framework over the
...
OUTPUTS :
  data/results/step5/ml_mu_ridge.csv
  data/results/step5/diagnostics/ridge/
    ridge_ic_detail.csv
...
DIAG_DIR    = os.path.join(STEP5_DIR, "diagnostics", "ridge")
...
def tune_ridge(X_tr, y_tr, X_vl, y_vl):
    """
    Grid-search alpha for Ridge on the validation set (RÂ²).
...
def run_oos(panel, oos_dates, returns, tickers, feature_cols):
    ...
    for i, t in enumerate(oos_dates):
        ...
        mdl, sc, alpha = tune_ridge(X_tr, y_tr, X_vl, y_vl)
        ...
        print(f"    Ridge   alpha={alpha:<6}  "
              f"t={time.perf_counter()-t0:.1f}s  "
              f"pred_std={scores.std():.4f}")

        hyper_rows.append({"date": t_str, "alpha": alpha})
...
def main():
    ...
    print("=" * 70)
    print("  STEP 5A â€” RIDGE REGRESSION EXPECTED RETURN ESTIMATION (MU_HAT)")
    print("=" * 70)
    ...
    out_path = os.path.join(STEP5_DIR, "ml_mu_ridge.csv")
  ...
  Next step: run step5b_xgboost.py or step5c_mlp.py.
""")
