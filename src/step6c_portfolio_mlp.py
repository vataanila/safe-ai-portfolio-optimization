...
=======================
PURPOSE : Step 6c â€” MLP ML-enhanced Markowitz MIQP portfolio construction
          and backtest.

          This script loads precomputed MLP mu_hat predictions from Step 5c
          ...
          The ONLY difference vs. step3_baseline_portfolio.py is the mu estimator:
            ...
            - MLP (Step 6c)     : MLP ML predictions from Step 5c
...
PIPELINE:
  ...
  Step 5c â†’ step5c_mlp_mu.py              (MLP mu predictions)
  Step 6a â†’ step6a_portfolio_ridge.py      (Ridge variant)
  Step 6b â†’ step6b_portfolio_xgboost.py    (XGBoost variant)
  Step 6c â†’ this script                    (MLP MIQP portfolio, backtested)
...
INPUTS  :
  ...
  data/results/step5/ml_mu_mlp.csv           -- MLP mu_hat (rebal dates Ã— tickers)

OUTPUTS (data/results/step6/mlp/) :
  mlp_weights.csv
  mlp_returns.csv
  mlp_turnover.csv
  mlp_summary.csv
  mlp_wealth_drawdown.csv
  mlp_net_cost_summary.csv
  mlp_portfolio_log.txt

METHODOLOGY:
  mu  : MLP ML predictions loaded from step5c â€” NOT recomputed here.
...
log("=" * 70)
log(f"  STEP 6c â€” MLP ML-ENHANCED MARKOWITZ MIQP PORTFOLIO")
log("=" * 70)
...
print("\n" + "=" * 70)
print(f"  STEP 6c â€” {MODEL_NAME.upper()} PORTFOLIO COMPLETE")
print("=" * 70)
