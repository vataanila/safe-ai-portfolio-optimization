"""
Takes the structurally-clean universe from 1a_load.py and computes daily
log returns (from TRI, to capture dividend income -- prices are kept only
for the Amihud illiquidity ratio) plus three full-window covariance
estimators (sample, Ledoit-Wolf, OAS) for descriptive purposes.

mu is NOT computed here -- the baseline uses a rolling historical mean
inside the optimizer itself, and the ML models get their mu from
4a_features.py + 5a/5b/5c. Same goes for the covariance matrices saved
here: they're full-model-window diagnostics only. The actual OOS
optimizer in 3a_baseline.py re-estimates Sigma at every rebalance date
using only past returns, following DeMiguel, Garlappi and Uppal (2009).

Also runs three universe filters on top of Step 1's structural cleaning:
a >5% zero-return filter (catches stale Bloomberg TRI feeds), one
hardcoded exclusion (SATS UW EQUITY, unverifiable return spike), and the
GOOGL/GOOG duplicate-share-class removal.

Writes returns.csv, the three Sigma_*.csv files, stock/correlation
diagnostics, and the vol/return/corr/sector figures.
...
from config import BASE_DIR, TRADING_DAYS

warnings.filterwarnings("ignore", category=FutureWarning)
pd.set_option("future.no_silent_downcasting", True)

RESULTS_DIR = os.path.join(BASE_DIR, "data", "clean")
FIGURES_DIR = os.path.join(BASE_DIR, "figures", "step2")
...
if tri_tickers != price_tickers:
    ...
    raise ValueError(
        ...
        f"  Only in prices: {only_price}\n"
        "Re-run step1.")
log("  Ticker alignment (tri/prices)  : OK")
...
if mktcap_tickers != price_tickers:
    ...
    raise ValueError(
        ...
        f"  Only in prices : {only_price}\n"
        "Re-run step1.")
log("  Ticker alignment (mkt_cap/prices): OK")
...
if actual_start != SAMPLE_START or actual_end != SAMPLE_END:
    ...
    log("  Date range       : matches expected window  OK")
...
log("  Return source       : TRI (total return index, gross dividends)")
...
to_exclude = [t for t in HARDCODED_EXCLUSIONS if t in returns.columns]
if to_exclude:
...
# GOOG/GOOGL are just two share classes of Alphabet (corr 0.9952 over the model
# window) -- keeping both would let the optimizer double up on the same company,
# so GOOGL goes and we keep GOOG (more liquid). Gu, Kelly & Xiu (2020) do the same.
returns_model  = returns.loc[MODEL_START:MODEL_END]
returns_warmup = returns.loc[SAMPLE_START:WARMUP_END]
...
# steps 3-6 all read from these same filenames
...
with open(log_path, "w", encoding="utf-8") as f:
    f.write("\n".join(log_lines))
print("  Log saved: preproc_log.txt")

log("\n-- 10. FINAL SUMMARY ------------------------------------------------")
log(f"""
  Dataset             : Clean universe (1a_load.py)
...
log("=" * 70)
log("  STEP 2 COMPLETE -- Run 3a_baseline.py next.")
log("=" * 70)
