"""
Ridge-driven counterpart of 3a_baseline.py: loads the precomputed Ridge
mu_hat from 5a_ridge.py and runs the same MIQP across the lambda grid to
build a portfolio frontier. Covariance estimator, cardinality/weight/sector
constraints, buy-and-hold return calc, and the 0.5*sum|w_target-w_pre_trade|
turnover convention are all unchanged from the baseline -- mu is the only
thing that differs.

6b (XGBoost) and 6c (MLP) are this same script with just the mu source and
output paths swapped, so this docstring covers all three; it isn't repeated
in the other two.

Reads: data/clean/returns.csv, data/clean/meta_clean.csv,
       data/results/step3/baseline_weights.csv,
       data/results/step5/ridge/predictions/ml_mu_ridge.csv
Writes (per lambda): data/results/step6/ridge/{weights,returns,performance,
       logs}/ridge_*_lambda_{tag}.csv, plus the aggregated
...
# ---------- model identity ---------------------------------------------------
MODEL_NAME = "ridge"
MU_FILE    = "ml_mu_ridge.csv"

# ---------- paths ------------------------------------------------------------
RESULTS_DIR      = os.path.join(BASE_DIR, "data", "results")
STEP3_DIR        = os.path.join(RESULTS_DIR, "step3")
STEP5A_PRED_DIR  = os.path.join(RESULTS_DIR, "step5", "ridge", "predictions")
STEP6_DIR        = os.path.join(RESULTS_DIR, "step6")
MODEL_STEP6_DIR  = os.path.join(STEP6_DIR, "ridge")

WEIGHTS_DIR      = os.path.join(MODEL_STEP6_DIR, "weights")
RETURNS_DIR      = os.path.join(MODEL_STEP6_DIR, "returns")
PERFORMANCE_DIR  = os.path.join(MODEL_STEP6_DIR, "performance")
LOG_DIR          = os.path.join(MODEL_STEP6_DIR, "logs")

for _d in [MODEL_STEP6_DIR, WEIGHTS_DIR, RETURNS_DIR, PERFORMANCE_DIR, LOG_DIR]:
...
log("=" * 70)
log("  STEP 6a - RIDGE ML-ENHANCED MARKOWITZ MIQP PORTFOLIO FRONTIER")
log("=" * 70)
...
mu_path = os.path.join(STEP5A_PRED_DIR, MU_FILE)
...
def compute_metrics(daily_ret_df: pd.DataFrame, turnover_df: pd.DataFrame) -> dict:
    """Performance metrics - identical convention to 3a_baseline.py."""
...
for lambda_value in LAMBDA_GRID:
    ...
    for rebal_date in rebal_dates:
        ...
        weights, status_str, obj_val, elapsed = solve_miqp(mu_row, Sigma_t, lambda_value)
    ...
    wfname = f"{MODEL_NAME}_weights_lambda_{lam_tag}.csv"
    weights_out.to_csv(os.path.join(WEIGHTS_DIR, wfname))
    log(f"  Saved: {wfname}  ({weights_out.shape[0]} dates x {weights_out.shape[1]} tickers)")

    rfname = f"{MODEL_NAME}_returns_lambda_{lam_tag}.csv"
    daily_ret_df.to_csv(os.path.join(RETURNS_DIR, rfname))
    log(f"  Saved: {rfname}  ({len(daily_ret_df)} rows)")

    tfname = f"{MODEL_NAME}_turnover_lambda_{lam_tag}.csv"
    turnover_df.to_csv(os.path.join(PERFORMANCE_DIR, tfname), index=False)
    log(f"  Saved: {tfname}  ({len(turnover_df)} rows)")

    sfname = f"{MODEL_NAME}_summary_lambda_{lam_tag}.csv"
    summary_row = {
        "model"            : MODEL_NAME,
        "lambda"           : lambda_value,
    ...
    pd.DataFrame([summary_row]).to_csv(os.path.join(PERFORMANCE_DIR, sfname), index=False)
    log(f"  Saved: {sfname}")

    wdfname = f"{MODEL_NAME}_wealth_drawdown_lambda_{lam_tag}.csv"
    wd_df = pd.DataFrame({
        "portfolio_return" : r,
        "wealth_index"     : wealth,
    ...
    wd_df.to_csv(os.path.join(PERFORMANCE_DIR, wdfname))
    log(f"  Saved: {wdfname}  ({len(wd_df)} rows)")

    ncfname = f"{MODEL_NAME}_net_cost_summary_lambda_{lam_tag}.csv"
    ...
    net_cost_df.to_csv(os.path.join(PERFORMANCE_DIR, ncfname), index=False)
    log(f"  Saved: {ncfname}  ({len(net_cost_df)} cost scenarios)")

    if lambda_value == 1.0:
        weights_out.to_csv(os.path.join(WEIGHTS_DIR,     f"{MODEL_NAME}_weights.csv"))
        daily_ret_df.to_csv(os.path.join(RETURNS_DIR,    f"{MODEL_NAME}_returns.csv"))
        turnover_df.to_csv(os.path.join(PERFORMANCE_DIR, f"{MODEL_NAME}_turnover.csv"), index=False)
        pd.DataFrame([summary_row]).to_csv(os.path.join(PERFORMANCE_DIR, f"{MODEL_NAME}_summary.csv"), index=False)
        wd_df.to_csv(os.path.join(PERFORMANCE_DIR,       f"{MODEL_NAME}_wealth_drawdown.csv"))
        net_cost_df.to_csv(os.path.join(PERFORMANCE_DIR, f"{MODEL_NAME}_net_cost_summary.csv"), index=False)
        log("  Also saved untagged copies for lambda=1.0 (required by step6d/6e)")

    log_path = os.path.join(LOG_DIR, f"{MODEL_NAME}_portfolio_log_lambda_{lam_tag}.txt")
    with open(log_path, "w", encoding="utf-8") as f:
        f.write("\n".join(log_lines))
    print(f"  Log saved: {MODEL_NAME}_portfolio_log_lambda_{lam_tag}.txt")
...
frontier_path = os.path.join(PERFORMANCE_DIR, f"{MODEL_NAME}_frontier_summary.csv")
frontier_df.to_csv(frontier_path, index=False)
print(f"\n  Saved: {MODEL_NAME}_frontier_summary.csv  ({len(frontier_df)} rows)")

print("\n" + "=" * 70)
print(f"  STEP 6a -- {MODEL_NAME.upper()} PORTFOLIO FRONTIER COMPLETE")
print("=" * 70)
...
print("=" * 70)
print(f"  Output directory : {MODEL_STEP6_DIR}")
print("=" * 70)
