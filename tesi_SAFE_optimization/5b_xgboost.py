"""
5b_xgboost.py -- Step 5b, mu_hat from XGBoost.

Same expanding-window OOS setup as step5a, but with XGBRegressor and a
3-way grid search (n_estimators in {100,300,500}, max_depth in {3,4,6},
learning_rate in {0.01,0.05}) selected by validation MSE with early
stopping, refit on train+val. Predictions get rescaled the same way as
Ridge's so both plug into the identical MIQP downstream.

Inputs: data/results/step4/ml_panel.csv, data/clean/returns.csv,
        data/results/step3/baseline_weights.csv
Outputs: data/results/step5/xgboost/predictions/ml_mu_xgboost.csv,
         plus IC/feature-importance diagnostics under
...
STEP5_DIR   = os.path.join(RESULTS_DIR, "step5", "xgboost")
...
def run_oos(panel, oos_dates, returns, tickers, feature_cols):
    ...
    for i, t in enumerate(oos_dates):
        ...
        model_path = os.path.join(MODEL_DIR, f"xgboost_model_{t_str}.pkl")
...
def main():
    ...
    feature_cols_path = os.path.join(MODEL_DIR, "xgboost_feature_cols.json")
    ...
    manifest_path = os.path.join(MODEL_DIR, "xgboost_saved_models_manifest.csv")
    ...
    out_path = os.path.join(PRED_DIR, "ml_mu_xgboost.csv")
  ...
  Models for Step 7D Explainability:
    data/results/step5/models/xgboost/xgboost_model_YYYY-MM-DD.pkl
    data/results/step5/models/xgboost/xgboost_feature_cols.json
    data/results/step5/models/xgboost/xgboost_saved_models_manifest.csv

  Next step: run 5a_ridge.py or 5c_mlp.py.
""")
