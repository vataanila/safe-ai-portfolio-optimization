"""
Efficient-frontier plot for the methodology chapter -- not tied to the
MIQP backtest, just a classical mean-variance picture for context.

Everything is estimated on the in-sample model window only (2016-01-01 to
2022-12-31, pre-test), with mu = annualised sample mean and Sigma =
annualised Ledoit-Wolf. Plots 500 random sparse portfolios (Dirichlet
weights, K~U[5,30] names), the min-variance and max-Sharpe portfolios
(both SLSQP), and the equal-weight point.

Reads data/clean/returns.csv, writes data/figures/efficient_frontier.png
(300 dpi, 10x7in).
...
from config import BASE_DIR, CLEAN_DIR, TRADING_DAYS

warnings.filterwarnings("ignore")

RESULTS_DIR = os.path.join(BASE_DIR, "data", "results")
FIGURES_DIR = os.path.join(BASE_DIR, "figures", "step3")
...
print(f"  Full series  : {returns_full.shape[0]} days × {returns_full.shape[1]} stocks")
print(f"  Model window : {MODEL_START} to {MODEL_END}")
print(f"  After dropna : {returns_df.shape[0]} days × {N} stocks")
...
ax.scatter(
    asset_vol, asset_ret,
    c="lightgrey", s=120, alpha=0.25, zorder=2,
...
ax.scatter(
    vol_mvp, ret_mvp,
    marker="*", s=120, color="#1f3c6b", zorder=7,
    edgecolors="white", linewidths=1,
    label=f"Min Variance  (Sharpe {sharpe_mvp:.2f})",
)
ax.scatter(
    vol_msp, ret_msp,
    marker="*", s=120, color="gold", zorder=7,
    edgecolors="white", linewidths=1,
    label=f"Max Sharpe    (Sharpe {sharpe_msp:.2f})",
)
ax.scatter(
    vol_ew, ret_ew,
    marker="D", s=120, color="#d62728", zorder=7,
    edgecolors="white", linewidths=1,
