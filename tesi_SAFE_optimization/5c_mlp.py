"""
Step 5c: mu_hat from a small MLP (sklearn MLPRegressor).

Unlike Ridge/XGBoost, the architecture here is fixed rather than
grid-searched: hidden_layer_sizes=(64, 32), alpha=0.001. Same expanding-
window OOS loop over baseline_weights.csv rebalancing dates, same
rescaling to the baseline mu distribution at the end.

Inputs : data/results/step4/ml_panel.csv, data/clean/returns.csv,
         data/results/step3/baseline_weights.csv
Outputs: data/results/step5/mlp/predictions/ml_mu_mlp.csv
         + training/IC diagnostics under data/results/step5/diagnostics/mlp/
...
STEP5_DIR   = os.path.join(RESULTS_DIR, "step5", "mlp")
...
def train_mlp(X_tr, y_tr, X_vl, y_vl):
    """
    Train MLP on train+val combined with sklearn early_stopping=True.
    sklearn internally reserves 10% of the combined data as a validation
    holdout for early stopping, this is asymmetric with Ridge and XGBoost,
    where val_dates are used explicitly for hyperparameter tuning.
    Here val_dates serve only to bound the eligible training window (no
    hyperparameter grid search is performed for MLP).
    No temporal leakage: all rows in X_tv have target_end_date <= t.
    Fixed architecture (64, 32), no hyperparameter search.
...
def run_oos(panel, oos_dates, returns, tickers, feature_cols):
    """
    Expanding-window OOS loop for MLPRegressor.
    ...
    """
    print(f"\n[2] OOS expanding-window loop (MLP) over {len(oos_dates)} dates ...")
    ...
    model_dir = Path(BASE_DIR) / "data" / "results" / "step5" / "mlp" / "models"
    ...
    for i, t in enumerate(oos_dates):
        ...
        mdl, sc = train_mlp(X_tr, y_tr, X_vl, y_vl)

        model_path  = model_dir / f"mlp_model_{t.date()}.pkl"
        scaler_path = model_dir / f"mlp_scaler_{t.date()}.pkl"
        ...
        print(f"    MLP     iters={mdl.n_iter_:<5}  "
              f"t={time.perf_counter()-t0:.1f}s  "
              f"pred_std={scores.std():.4f}")

        train_rows.append({"date": t_str, "n_iter": int(mdl.n_iter_)})
    ...
    manifest.to_csv(model_dir / "mlp_saved_models_manifest.csv", index=False)
    manifest[["date", "scaler_path"]].to_csv(
        model_dir / "mlp_scaler_manifest.csv", index=False
    )

    with open(model_dir / "mlp_feature_cols.json", "w") as f:
        json.dump(feature_cols, f)

    return mu_hat_dict, diag_rows, train_rows
...
def save_diagnostics(diag_rows, train_rows, mu_mat, oos_dates, returns):
    """Compute and save all MLP diagnostics to DIAG_DIR."""
    ...
    # -- 1. IC detail ----------------------------------------------------------
    diag_df.to_csv(os.path.join(DIAG_DIR, "mlp_ic_detail.csv"), index=False)
    print("  Saved: mlp_ic_detail.csv")
    ...
    ic_df.to_csv(os.path.join(DIAG_DIR, "mlp_ic_summary.csv"), index=False)
    print("  Saved: mlp_ic_summary.csv")
    ...
    pd.DataFrame(corr_rows).to_csv(
        os.path.join(DIAG_DIR, "mlp_mu_baseline_correlation.csv"), index=False
    )
    print(f"  Saved: mlp_mu_baseline_correlation.csv ({len(corr_rows)} rows)")
    ...
    pd.DataFrame(summ_rows).to_csv(
        os.path.join(DIAG_DIR, "mlp_prediction_summary.csv"), index=False
    )
    print(f"  Saved: mlp_prediction_summary.csv ({len(summ_rows)} rows)")

    # -- 5. Training summary (n_iter per date) ---------------------------------
    pd.DataFrame(train_rows).to_csv(
        os.path.join(DIAG_DIR, "mlp_training_summary.csv"), index=False
    )
    print(f"  Saved: mlp_training_summary.csv ({len(train_rows)} rows)")
...
def main():
    for d in [STEP5_DIR, PRED_DIR, DIAG_DIR]:
        os.makedirs(d, exist_ok=True)

    print("=" * 70)
    print("  STEP 5C - MLP EXPECTED RETURN ESTIMATION (MU_HAT)")
    print("=" * 70)

    panel, returns, baseline_weights, oos_dates, tickers, feature_cols = load_inputs()

    mu_hat_dict, diag_rows, train_rows = run_oos(
    ...
    out_path = os.path.join(PRED_DIR, "ml_mu_mlp.csv")
    mu_mat.to_csv(out_path)
    print(f"  Saved: ml_mu_mlp.csv  shape={mu_mat.shape}  "
          f"NaN remaining={int(mu_mat.isna().sum().sum())}")

    save_diagnostics(diag_rows, train_rows, mu_mat, oos_dates, returns)
  ...
  Diagnostics ({DIAG_DIR}):
    mlp_ic_detail.csv
    mlp_ic_summary.csv
    mlp_mu_baseline_correlation.csv
    mlp_prediction_summary.csv
    mlp_training_summary.csv

  Next step: run 5a_ridge.py or 5b_xgboost.py.
""")
