"""
Diagnostic figures for the baseline portfolio from 3a_baseline.py: cumulative
returns against the S&P 500 (pulled live via yfinance if available, skipped
otherwise), 126-day rolling Sharpe, drawdown, monthly turnover, and a GICS
sector-allocation heatmap over time.

Reads baseline_returns/weights/turnover.csv from data/results/step3/ and
meta_clean.csv for sector labels. Writes five PNGs to data/figures/.
...
RESULTS_DIR = os.path.join(BASE_DIR, "data", "results", "step3")
...
print(f"  baseline_weights : {weights_df.shape[0]} rebal dates × "
...
if not os.path.exists(turnover_path):
    raise FileNotFoundError(
        f"Required file not found: {turnover_path}\n"
        "Run 3a_baseline.py first.")
...
try:
    ...
    plt.bar(dates, to_val, color=colors, width=15, align="center",
            alpha=0.70, edgecolor="none", zorder=3)
