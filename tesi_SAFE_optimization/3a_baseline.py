"""
Classical Markowitz MIQP baseline: trailing historical mean as mu, rolling
Ledoit-Wolf covariance, monthly rebalancing. This is the benchmark every ML
model gets compared against, so every other design choice (solver,
constraints, Sigma estimator) is held fixed in the ML steps -- mu is the
only thing that changes.

MIQP: minimize w'Sigma*w - lambda*mu'w, s.t. sum(w)=1, cardinality K=10,
weight bounds [1%, 20%] on selected names, sector cap 30%. Solved with
Gurobi (MVar API), TimeLimit=60s, MIPGap=0.01, monthly rebalancing over
2023-01-01 to 2025-12-31. Both mu and Sigma at each rebalance date use only
trailing ESTIM_WINDOW-day returns strictly before that date -- Step 2's
full-window covariance files are diagnostics only and never touch the
optimizer here.

Also runs three implementability/sustainability diagnostics that don't
affect the portfolio itself: wealth/drawdown curve with peak-trough dates,
an equal-weight benchmark, and transaction-cost sensitivity at 10/20/30 bps.

Reads: data/clean/returns.csv, data/clean/meta_clean.csv
Writes: data/results/step3/baseline_{weights,returns,turnover,summary,
        wealth_drawdown}.csv, equal_weight_{returns,summary}.csv,
...
from config import BASE_DIR, CLEAN_DIR, ESTIM_WINDOW, TRADING_DAYS, W_MAX, W_MIN
...
LAMBDA       = 1.0     # risk-return tradeoff: minimize w'Σw - λ·μ'w
...
for fname in ["returns.csv", "meta_clean.csv"]:
    ...
    if not os.path.exists(path):
        raise FileNotFoundError(
            ...
            "Run 1a_load.py and 2a_preprocess.py first.")
...
log(f"  returns      : {returns_df.shape[0]} days × {returns_df.shape[1]} stocks")
...
log("  Rebalancing    : first trading day of each calendar month")
...
log("\n-- 5. OPTIMIZATION LOOP ---------------------------------------------")
log(f"  Objective  : minimize w'Σw - λ·μ'w  (λ={LAMBDA})")
log(f"  K={K}, w∈[{W_MIN:.0%},{W_MAX:.0%}], sector cap={SECTOR_CAP:.0%}")
log(f"  μ estimator: trailing {ESTIM_WINDOW}-day mean × {TRADING_DAYS}, winsorized p1/p99")
log(f"  Σ estimator: rolling Ledoit-Wolf, re-estimated at each rebalancing date"
    f" using trailing {ESTIM_WINDOW} returns (same window as mu)")
log(f"  Gurobi     : TimeLimit={TIME_LIMIT}s, MIPGap={MIP_GAP}")
log("  Out-of-sample design: both mu and Sigma are re-estimated at each rebalance"
    " date using only past returns, following the rolling-window evaluation logic"
    " of DeMiguel, Garlappi and Uppal (2009).")
log("-" * 70)
...
for i, rebal_date in enumerate(rebal_list):
    ...
    for date, row in period_ret.iterrows():
        ...
        # each stock's weight grows proportionally to its own return, not reset until next rebalance
...
# prepend 1.0 so drawdown from day one is captured, not just after the first return
...
log("  Rebalancing: monthly (same dates as baseline); weights drift between")
log("  rebalancing dates -- identical convention to the baseline portfolio.")
...
log(f"  Saved: baseline_weights.csv  "
    f"({weights_df.shape[0]} dates × {weights_df.shape[1]} tickers)")
...
with open(log_path, "w", encoding="utf-8") as f:
    f.write("\n".join(log_lines))
print("  Log saved: baseline_log.txt")
...
""")
log("=" * 70)
log("  Next: run 4a_features.py, then 5a_ridge.py / 5b_xgboost.py / 5c_mlp.py for mu_hat.")
log("=" * 70)
